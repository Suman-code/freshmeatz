...
$(document).on("click", ".delete-wishlist-item", function () {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      data = response.data
      document.getElementById('wishlistCount').innerHTML = "My wishlist" + " " + data.total_wishlist
        ...
        if (data.status == "address added") {
          ...
          for (i = 0; i < x.length; i++) {
            output +=
              ...
              x[i].state +
              x[i].pincide +
              "</small> <input class='btn fw-bold btn btn-sm edit-btn text-danger'  value='EDIT' style='font-size: 12px; float: right;] data-sid=" +
              x[i].id +
              ">";
          }
